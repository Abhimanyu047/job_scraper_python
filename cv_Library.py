...
url = 'https://www.cv-library.co.uk/data-scientist-jobs-in-london?us=1'
print(f"url:{url}")
...
all_jobs = job_soup.find('ol',{'id':'searchResults'}).find_all('li',class_='results__item')

# For CV-Library jobs
# Job Title
titles_list = []
for i in range(0,len(all_jobs)):
    title = all_jobs[i].find(['h2'],{'class':'job__title'}).text.strip()
    titles_list.append(title.strip())

# Company Name
company_list = []
for i in range(0,len(all_jobs)):
    company = all_jobs[i].find('div',{'class':'job__posted-row'}).find('a').text
    company_list.append(company)

# Link to the job
job_links = []
for i in range(0,len(all_jobs)):
    job_link = 'https://cv-library.co.uk' + all_jobs[i].find(['h2'],{'class':'job__title'}).find('a').get('href')
    job_links.append(job_link)

# Date
job_dates = []
for i in range(0,len(all_jobs)):
    job_date = 'Posted: ' + all_jobs[i].find(['p'],{'class':'job__posted-by'}).find(['span'],{'data-jd-posted':''}).text
    job_dates.append(job_date)

# short Job Description
job_desc_short = []
for i in range(0,len(all_jobs)):
    job_desc_shrt = ' '.join(all_jobs[i].find(['p'],{'class':'job__description'}).text.strip().split())
...
for job_number in range(0,len(df_jobs['Job Link'])):
    ...
    time.sleep(3)
    job_details = job_soup.find('div',class_='job__description')
    time.sleep(2)
