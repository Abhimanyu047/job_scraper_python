...
url = 'https://www.reed.co.uk/jobs/data-scientist-jobs-in-london'
print(f"url:{url}")
...
all_jobs = job_soup.find(['section'],{'class':'results'}).find_all(['article'],{'class':'job-result-card'})

# extra info
salary = all_jobs[0].find('ul',{'class':'job-metadata'}).find(['li'],{'class':'job-metadata__item--salary'}).text.strip()

# For Reed.co.uk jobs
# Job Title
titles_list = []
for i in range(0,len(all_jobs)):
    title = all_jobs[i].find(['h3'],{'class':'job-result-heading__title'}).text.strip()
    titles_list.append(title.strip())

# Company Name
company_list = []
for i in range(0,len(all_jobs)):
    company = all_jobs[i].find('div',{'class':'job-result-heading__posted-by'}).find('a').text.strip()
    company_list.append(company)

# Link to the job
job_links = []
for i in range(0,len(all_jobs)):
    job_link = 'https://reed.co.uk/' + all_jobs[i].find(['h3'],{'class':'job-result-heading__title'}).find('a').get('href')
    job_links.append(job_link)

# Date
job_dates = []
for i in range(0,len(all_jobs)):
    company_name = all_jobs[i].find('div',{'class':'job-result-heading__posted-by'}).find('a').text.strip()
    job_date = 'Posted: ' + all_jobs[i].find('div',{'class':'job-result-heading__posted-by'}).text.strip().replace(company_name,'').replace('by','').strip()
    job_dates.append(job_date)

# short Job Description
job_desc_short = []
for i in range(0,len(all_jobs)):
    job_desc_shrt = ' '.join(all_jobs[i].find(['div'],{'class':'job-result-description'}).text.strip().replace('\n','').split())
...
for job_number in range(0,len(df_jobs['Job Link'])):
    ...
    time.sleep(3)
    job_details = job_soup.find('div',class_='branded-job--description-container')
    time.sleep(2)
...
df_final['Skills'] = df_final['Skills'].apply(lambda val: ' '.join(val.split()))
df_final['Responsibilities'] = df_final['Responsibilities'].apply(lambda val: ' '.join(val.split()))
df_final['Additional_Q'] = df_final['Additional_Q'].apply(lambda val: ' '.join(val.split()))
df_final['Job Summary'] = df_final['Job Summary'].apply(lambda val: ' '.join(val.split()))
